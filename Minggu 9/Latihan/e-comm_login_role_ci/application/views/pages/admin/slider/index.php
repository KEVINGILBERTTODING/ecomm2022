	...
	<div class="card-body">
		...
		<div class="row my-2">
			<div class="col-md-8 col-sm-12">
				<a href="<?= base_url('slider/create') ?>" class="btn btn-sm btn-secondary">Tambah</a>
			</div>
			<div class="col-md-2 col-sm-12">
				<?= form_open('slider/search', ['method' => 'POST', 'class' => 'form-inline']) ?>
				<div class="form-group">
					...
					<div class="btn-group mx-sm-3" role="group" aria-label="Basic example">
						...
						<a href="<?= base_url('slider/reset') ?>" class="btn btn-sm btn-secondary"><i class="fa fa-fw fa-eraser"></i></a>
					</div>
				</div>
				<?= form_close() ?>
			</div>
				...
				<tbody>
					<?php foreach ($content as $row) : ?>
						<tr>
							...
							<td><?= $row->title ?></td>
							<td>
								<?= form_open(base_url("slider/delete/$row->id"), ['method' => 'POST']) ?>
								<?= form_hidden('id', $row->id) ?>
								<a href="<?= base_url("slider/edit/$row->id") ?>" class="btn btn-sm"><i class="fas fa-fw fa-edit text-info"></i></a>
